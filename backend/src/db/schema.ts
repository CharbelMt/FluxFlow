...
export const users = pgTable(
	...
	(table) => [
		...
		check(
			"users_role_check",
			sql`(role)::text = ANY ((ARRAY['admin'::character varying, 'manager'::character varying, 'supervisor'::character varying])::text[])`,
		),
	],
);
...
export const siteSupervisors = pgTable(
	"site_supervisors",
	{
		id: uuid()
			.default(sql`uuid_generate_v4()`)
...
export const storageRooms = pgTable(
	"storage_rooms",
	{
		id: uuid()
			.default(sql`uuid_generate_v4()`)
...
export const auditLogs = pgTable(
	"audit_logs",
	{
		...
		serverSyncedAt: timestamp("server_synced_at", { mode: "string" }).default(
			sql`CURRENT_TIMESTAMP`,
		),
	...
	(table) => [
		index("regression_index").using(
			"btree",
			table.assetId.asc().nullsLast().op("timestamp_ops"),
			table.clientCreatedAt.asc().nullsLast().op("timestamp_ops"),
		),
		foreignKey({
		...
		check(
			"audit_logs_condition_score_check",
			sql`(condition_score >= 1) AND (condition_score <= 5)`,
		),
	],
);
